...
class OneCClient:
    ...
    async def create_order(self, order_data: Dict) -> Dict:
        ...
        for i, product in enumerate(products, 1):
            ...
            nom_data = await self._get_nomenclature_by_article(product.get("article", ""))
            if not nom_data:
...
<entry xmlns="http://www.w3.org/2005/Atom" xmlns:d="http://schemas.microsoft.com/ado/2007/08/dataservices" xmlns:m="http://schemas.microsoft.com/ado/2007/08/dataservices/metadata">
  <content type="application/xml">
    ...
    async def _get_nomenclature_by_article(self, article: str) -> Optional[Dict]:
        """Получить номенклатуру с НДС"""
        try:
            url = f"{self.odata_url}/Catalog_%D0%9D%D0%BE%D0%BC%D0%B5%D0%BD%D0%BA%D0%BB%D0%B0%D1%82%D1%83%D1%80%D0%B0?$filter=Артикул%20eq%20%27{article}%27&$select=Ref_Key,СтавкаНДС_Key"
            response = await self.client.get(url)
            if response.status_code == 200 and 'Ref_Key' in response.text:
